...
try {
  ...
  const { rows: groups } = await db.query<{ id: string; name: string; is_joker: boolean }>(
    `select id, name, is_joker from groups where tournament_id=$1 order by sort_order`, [t.id])
  const jokerGroup = groups[0]!
  const normalGroup = groups[1]!

  const { rows: members } = await db.query<{ id: string; display_name: string }>(
    `select id, display_name from tournament_members where tournament_id=$1`, [t.id])
  ...
  await db.query(`update tournament_members set group_id=$1 where id = any($2)`,
    [jokerGroup.id, [memberOf('선수1'), memberOf('선수2')]])
  await db.query(`update tournament_members set group_id=$1 where id = any($2)`,
    [normalGroup.id, [memberOf('선수3'), memberOf('선수4')]])
  ...
  const outsiderScore = await rpc(outsider.token, 'record_score', {
    p_match_id: matchId, p_side: 'A', p_delta: 1, p_client_event_id: 'outsider-attempt-1',
  })
  check('배정되지 않은 사람은 점수를 못 넣는다', outsiderScore.status >= 400,
    `status=${outsiderScore.status}`)

  console.log('\n── 진행 ──')
  const notLive = await rpc(ref.token, 'record_score', {
    p_match_id: matchId, p_side: 'A', p_delta: 1, p_client_event_id: 'before-start-1',
  })
  ...
  for (let i = 1; i <= 10; i++) {
    await rpc(ref.token, 'record_score', {
      p_match_id: matchId, p_side: 'A', p_delta: 1, p_client_event_id: `evt-alpha-${String(i).padStart(3, '0')}`,
    })
  }
  const at10 = await db.query<{ score_a: number; status: string }>(
    `select score_a, status from matches where id=$1`, [matchId])
  check('조커조 10점에서는 아직 안 끝난다',
  ...
  for (let i = 0; i < 5; i++) {
    await rpc(ref.token, 'record_score', {
      p_match_id: matchId, p_side: 'A', p_delta: 1, p_client_event_id: 'evt-alpha-010',
    })
  }
  const afterRetry = await db.query<{ score_a: number }>(
    `select score_a from matches where id=$1`, [matchId])
  check('같은 요청이 5번 더 도착해도 점수는 그대로다',
    afterRetry.rows[0]!.score_a === 10, `${afterRetry.rows[0]!.score_a}점`)

  console.log('\n── 취소 ──')
  await rpc(ref.token, 'undo_score', { p_match_id: matchId })
  const afterUndo = await db.query<{ score_a: number }>(
    `select score_a from matches where id=$1`, [matchId])
  check('취소하면 한 점 내려간다', afterUndo.rows[0]!.score_a === 9,
  ...
  const { rows: ledger } = await db.query<{ n: string; voided: string }>(
    `select count(*)::text n, count(*) filter (where voided)::text voided
     from score_events where match_id=$1`, [matchId])
  check('원장은 지우지 않고 무효 표시만 한다 (감사 추적)',
    Number(ledger[0]!.n) === 10 && Number(ledger[0]!.voided) === 1,
    `이벤트 ${ledger[0]!.n}개 중 ${ledger[0]!.voided}개 무효`)

  console.log('\n── 종료 (비대칭 목표) ──')
  await rpc(ref.token, 'record_score', {
    p_match_id: matchId, p_side: 'A', p_delta: 1, p_client_event_id: 'evt-alpha-redo-010',
  })
  const stillLive = await db.query<{ status: string }>(`select status from matches where id=$1`, [matchId])
  check('다시 10점이어도 안 끝난다', stillLive.rows[0]!.status === 'live')

  await rpc(ref.token, 'record_score', {
    p_match_id: matchId, p_side: 'A', p_delta: 1, p_client_event_id: 'evt-alpha-011',
  })
  const finished = await db.query<{ status: string; winner_side: string; score_a: number; score_b: number }>(
    `select status, winner_side, score_a, score_b from matches where id=$1`, [matchId])
  check('조커조가 11점에 닿는 순간 끝나고 승리한다',
  ...
  const afterFinish = await rpc(ref.token, 'record_score', {
    p_match_id: matchId, p_side: 'B', p_delta: 1, p_client_event_id: 'evt-bravo-after-finish',
  })
  check('끝난 경기에는 점수를 더 넣을 수 없다', afterFinish.status >= 400,
  ...
  const reopened = await db.query<{ status: string; score_a: number }>(
    `select status, score_a from matches where id=$1`, [matchId])
  check('마지막 한 점이 잘못 들어갔으면 심판이 2분 안에 되돌릴 수 있다',
    undoAfterFinish.status === 200 && reopened.rows[0]!.status === 'live' && reopened.rows[0]!.score_a === 10,
    `${reopened.rows[0]!.score_a}점 / ${reopened.rows[0]!.status}`)

  // 다시 끝내고 순위 확인
  await rpc(ref.token, 'record_score', {
    p_match_id: matchId, p_side: 'A', p_delta: 1, p_client_event_id: 'evt-alpha-final',
  })

  console.log('\n── 순위 집계 ──')
  const standings = await rpc(admin.token, 'get_standings', { p_tournament_id: t.id })
  const rows = standings.body as unknown as {
    group_name: string; is_joker: boolean; played: number; wins: number
    losses: number; points: string; scored: number; conceded: number; diff: number
  }[]
  const joker = rows.find((r) => r.is_joker)!
  const normal = rows.find((r) => !r.is_joker)!

  check('조커조가 이겼는데 승점은 0.5점이다',
  ...
  check('득실차가 맞다', joker.diff === joker.scored - joker.conceded && joker.scored === 11,
  ...
  const ledgerSum = await db.query<{ s: string }>(
    `select coalesce(sum(delta),0)::text s from score_events
     where match_id=$1 and side='A' and not voided`, [matchId])
  check('원장 합계와 화면에 보이는 점수가 일치한다',
