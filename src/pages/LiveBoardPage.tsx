  ...
  return (
    <div data-theme="dark" className="min-h-dvh bg-surface-0 p-4">
      {/*
        ...
        아래로 길어져서 출구가 위로 사라진다. 여기도 머리말을 붙여 둔다.
        껍데기가 `p-4` 라 `px-5 pt-6` 을 전제로 한 기본값을 여기에 맞춘다.

        하단탭이 없는 화면이라(전체화면 관전판) 이 링크가 유일한 출구다.
        글자는 '나가기' 에서 목적지로 바꿨다 — 어디로 나가는지 적힌 편이
        낫다(`BackLink` 주석).
      */}
      <BackBar to={`/t/${id}`} label="대회로" className="-mx-4 -mt-4 gap-3 px-4" topPad="1rem">
        {/*
