...
import type { Database, PlayerGender, PlayerGrade } from '@/types/database'
...
export async function joinAsGuest(
  ...
  const res = await guestSupabase.rpc('join_as_guest', {
    ...
    p_name: name,
    /*
     * 안 고른 값은 키 자체를 안 보낸다. p_grade · p_gender 는 서버에서 둘 다
     * 맨 뒤 `default null` 이라 안 보내면 그대로 null 이 들어간다 — 그리고
     * **그 경로가 곧 옛 3인자·4인자 호출이 안 깨진다는 증거**다. PostgREST 는
     * 함수를 인자 이름 집합으로 찾으므로 이 분기가 실제로 세 시그니처를
     * 다 시험한다.
     */
    ...(grade ? { p_grade: grade } : {}),
    ...(gender ? { p_gender: gender } : {}),
  })
