...
검증        단위 409(28파일) · 실DB 10종 전부 통과 (아래 "검증 명령")
브랜치      feat/session-mode (4b205ff)
```
...
- [ ] **즉석 모임(`starts_at is null`)은 시각 창을 무조건 통과합니다.**
      `guest_sessions` · `join_as_guest` · `guest_board` 세 함수의 시각 창(뒤 12시간 ~
      앞 24시간)이 `starts_at` 이 NULL 이면 판단할 것이 없어 그대로 통과시킵니다.
      **마일스톤 3 부터의 동작이고 마일스톤 4 가 새로 연 표면이 아닙니다** —
      읽기 필터는 등록 필터의 정확한 상위집합이라 창은 글자 그대로 같습니다.
      `coalesce(starts_at, created_at)` 이 옳지만, 그러면 **등록 필터도 같이 고쳐야
      상위집합이 유지되고** 마일스톤 3 회귀 25항목이 통째로 다시 필요합니다.
      좁히다 어긋나면 "등록은 됐는데 현황판이 안 보인다" — 코트 앞에 선 게스트를
      실제로 막는, 가장 나쁜 실패 모드입니다
...
```bash
npm run verify              # typecheck + lint + test + 단위 409 + CSP + build
